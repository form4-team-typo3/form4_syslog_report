...
class ReportCommand extends Command
{
    ...
    {
        $this->setDescription('Send a sys_log report by email for the configured time window and filters.')
            ->addArgument(
                'period',
    ...
    public function __construct(ViewFactoryInterface $viewFactory, Mailer $mailer, LogEntryRepository $logEntryRepository, LogService $logService)
    {
    ...
    {
        $subject = $input->getArgument('subject') ?: 'SysLog report from %s to %s';
        ...
        $view = $this->viewFactory->create(new ViewFactoryData(
            templateRootPaths: [GeneralUtility::getFileAbsFileName('EXT:form4_syslog_report/Resources/Private/Templates/')],
            partialRootPaths: [GeneralUtility::getFileAbsFileName('EXT:form4_syslog_report/Resources/Private/Partials/')],
            templatePathAndFilename: GeneralUtility::getFileAbsFileName('EXT:form4_syslog_report/Resources/Private/Templates/Report/Mail.html'),
        ));
    ...
    {
        $startTS = $this->logService->getStartTimeStamp();
        $endTS = $this->logService->getEndTimeStamp();
        // process the subject: substitute start and end date for placeholders and prepend the sitename
        $subject = sprintf($subject, date('d.m.Y H:i:s', $startTS), date('d.m.Y H:i:s', $endTS));
        ...
        if ($receiverEmail && $subject) {
            ...
            $mail->from(new Address($senderEmail, $senderName));
            $mail->to(new Address($receiverEmail));
            $mail->html($content);
            try {
