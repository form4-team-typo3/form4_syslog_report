...
class ReportCommand extends Command
{
    ...
    ): int
    {
        ...
        $view->setTemplateRootPaths([
            GeneralUtility::getFileAbsFileName('EXT:form4_syslog_report/Resources/Private/Templates/')
        ]);
        $view->setPartialRootPaths([
            GeneralUtility::getFileAbsFileName('EXT:form4_syslog_report/Resources/Private/Partials/')
        ]);
        $view->setTemplatePathAndFilename(
            GeneralUtility::getFileAbsFileName('EXT:form4_syslog_report/Resources/Private/Templates/Report/Mail.html')
        );
    ...
    protected function sendMail(string $receiverEmail, string $content, string $subject): bool
    {
        ...
        if ($receiverEmail && $subject) {
            ...
            $mail->from(GeneralUtility::makeInstance(Address::class, $senderEmail, $senderName));
            $mail->to(GeneralUtility::makeInstance(Address::class, $receiverEmail));
            $mail->html($content);
            $mail->send();
            $success = $mail->isSent();
        } else {
